@forelse($threads as $thread)
    <a href="{{ route('central.support.index', ['thread' => $thread->id]) }}" class="mb-2 block rounded-2xl border px-2 py-2 transition {{ $activeThread?->id === $thread->id ? 'border-sky-200 bg-sky-50/70' : 'border-slate-200 bg-white hover:bg-slate-50' }}">
        <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                    <p class="truncate text-xs font-semibold text-slate-900">{{ $thread->subject }}</p>
...
@empty
    <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-3 py-4 text-center text-xs text-slate-500">
