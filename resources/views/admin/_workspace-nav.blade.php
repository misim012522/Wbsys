    ...
    <div class="border-t border-slate-200 bg-slate-50/80 px-6 py-4">
        <div class="flex flex-wrap gap-2">
            ...
            @if($viewer?->isTenantAdmin())
                <a href="{{ route('admin.roles.index') }}" class="rounded-full px-4 py-2.5 text-sm font-medium {{ request()->routeIs('admin.roles.*') ? 'bg-emerald-600 text-white shadow-sm' : 'border border-slate-300 bg-white text-slate-700 hover:bg-slate-50' }}">Roles & permissions</a>
