...
<div class="mx-auto max-w-xl">
    <div class="rounded-[1.5rem] border border-slate-200 bg-white/50/50 p-6 shadow-sm">
        ...
        <form action="{{ route('admin.users.approve', $user) }}" method="POST" class="mt-4">
            ...
            <div class="flex gap-2">
                <button type="submit" class="rounded-[1.75rem] bg-emerald-600 px-4 py-2 text-sm text-white">Confirm approval</button>
                <a href="{{ route('admin.users.pending') }}" class="rounded-[1.75rem] border px-4 py-2 text-sm">Cancel</a>
            </div>
        </form>
    </div>
</div>
