...
<div class="-mx-4 -my-8 flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-sky-50 px-4 py-16 sm:px-6">
    <div class="w-full max-w-[420px]">
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white p-8 shadow-xl shadow-slate-200/50 ring-1 ring-slate-100 sm:p-10">
            ...
            <p class="mt-1 text-sm text-slate-500">Sign in to manage your office queue and appointments.</p>
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-5">
                ...
                @endif
                <button type="submit"
                    class="w-full rounded-xl bg-emerald-600 px-4 py-3.5 font-semibold text-white shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-700 hover:shadow-emerald-500/30 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 active:scale-[0.99]">
                    Log in
                </button>
            </form>

            <p class="mt-4 text-center text-sm text-slate-500">
                <a href="{{ route('password.request') }}" class="font-medium text-emerald-600 hover:text-emerald-700 hover:underline">Forgot password?</a>
            </p>
            <p class="mt-2 text-center text-sm text-slate-500">
                Office staff? <a href="{{ route('register') }}" class="font-medium text-emerald-600 hover:text-emerald-700 hover:underline">Create your account</a> if you don’t have one.
            </p>
        </div>
