...
<div class="flex items-center justify-between mb-8">
    ...
    <div class="flex gap-2 flex-wrap">
        <a href="{{ route('office.qr') }}" class="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700">QR code</a>
        <a href="{{ route('office.reports') }}" class="px-4 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 text-sm">Reports</a>
    ...
    <div>
        ...
        @if($currentServing)
            <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-4">
                ...
                <p class="text-xs mt-1 text-slate-600">Contact: @if($currentServing->guest_email)<a href="mailto:{{ $currentServing->guest_email }}" class="text-blue-600 hover:underline">{{ $currentServing->guest_email }}</a>@endif @if($currentServing->guest_email && $currentServing->guest_phone) · @endif @if($currentServing->guest_phone)<a href="tel:{{ $currentServing->guest_phone }}" class="text-blue-600 hover:underline">{{ $currentServing->guest_phone }}</a>@endif @if(!$currentServing->guest_email && !$currentServing->guest_phone)—@endif</p>
                <form method="POST" action="{{ route('office.queue.update', $currentServing) }}" class="mt-2 flex gap-2">
                    @csrf
                    @method('PATCH')
                ...
                <form method="POST" action="{{ route('office.queue.update', $currentServing) }}" class="inline mt-2">
                    @csrf
                    @method('PATCH')
                ...
                @forelse($todayQueue as $q)
                    <li class="px-4 py-3 flex items-center justify-between {{ $q->status === 'called' ? 'bg-amber-50' : '' }}">
                        ...
                        <div class="flex gap-2 shrink-0">
                            @if($q->status === 'waiting')
                                <form method="POST" action="{{ route('office.queue.update', $q) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                ...
                @forelse($todayAppointments as $a)
                    <li class="px-4 py-3 flex items-center justify-between">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">Contact: @if($a->guest_email)<a href="mailto:{{ $a->guest_email }}" class="text-blue-600 hover:underline">{{ $a->guest_email }}</a>@endif @if($a->guest_email && $a->guest_phone) · @endif @if($a->guest_phone)<a href="tel:{{ $a->guest_phone }}" class="text-blue-600 hover:underline">{{ $a->guest_phone }}</a>@endif @if(!$a->guest_email && !$a->guest_phone)—@endif</p>
                        </div>
                        <div class="flex gap-2 shrink-0">
                            @if($a->status === 'pending')
                                <form method="POST" action="{{ route('office.appointments.accept', $a) }}" class="inline">
                                    @csrf
                            ...
                            @if(in_array($a->status, ['pending', 'confirmed']))
                                <form method="POST" action="{{ route('office.appointments.complete', $a) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xs px-2 py-1 rounded bg-slate-100 text-slate-700">Complete</button>
                                </form>
                                <form method="POST" action="{{ route('office.appointments.cancel', $a) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xs px-2 py-1 rounded bg-red-100 text-red-700">Cancel</button>
                                </form>
                            @endif
