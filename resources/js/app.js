...
const initializeAppUi = () => {
    ...
    if (window.queueData) {
        ...
        const fetchStatusUpdate = () => {
            ...
            .then(data => updateStatusOnPage(data))
            .catch(error => console.error('Failed to fetch queue update:', error));
        };

        if (window.Echo) {
            // Use public channel so guests can join without being logged in
            window.Echo.channel(`tenant.public.${tenantId}`)
                .listen('.queue.updated', (e) => {
                    // Only update if this queue entry is affected OR if it's a general update
                    if (e.queue_entry_id === queueEntryId || e.action === 'updated' || e.action === 'called') {
                        fetchStatusUpdate();
                    }
                });
        }

        // Always add a polling fallback (every 10 seconds) for reliability on mobile devices
        window.setInterval(fetchStatusUpdate, 10000);
    }
